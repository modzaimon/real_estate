...
@section('content')
    ...
    <div class="card card-primary card-outline">
        <div class="card-header">
            <h3 class="card-title">{{ ucfirst($page)}} List <a href="{{ route('estate.create') }}"><span
                    ...
                    @else
                        @foreach ($data as $key => $val)
                            <tr>
                                ...
                                <td>{{ isset($val->estate_type->name)?$val->estate_type->name:'' }}</td>
                                <td>{{ $val->price }}</td>
                                <td>{{ $val->size }}</td>
                                <td>{{ $val->build_year }}</td>
                                <td>

                                    <form action="{{ route('estate.destroy', $val->id) }}" method="POST">
                                        <a href="{{ route('estate.show', $val->id) }}" class="btn btn-primary "> <i
                                                class="fa-sharp fa-solid fa-magnifying-glass">&nbsp;</i>View</a>
                                        <a href="{{ route('estate.edit', $val->id) }}" class="btn btn-warning"> <i
