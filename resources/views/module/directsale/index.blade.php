...
@section('content')
    ...
    <div class="card card-primary card-outline">
        <div class="card-header">
            <h3 class="card-title">List <a href="{{ route('directsale.create') }}"><span
                    ...
                    @else
                        @foreach ($data as $key => $val)
                            <tr>
                                ...
                                <td>{{ $val->name }}</td>
                                <td>{{ $val->tel }}</td>
                                <td>{{ isset($val->bank)?$val->bank->name:'' }}</td>
                                <td>

                                    <form action="{{ route('directsale.destroy', $val->id) }}" method="POST">
                                        <a href="{{ route('directsale.show', $val->id) }}" class="btn btn-primary "> <i
                                                class="fa-sharp fa-solid fa-magnifying-glass">&nbsp;</i>View</a>
                                        <a href="{{ route('directsale.edit', $val->id) }}" class="btn btn-warning"> <i
