...
@section('content')
    ...
    <div class="card card-primary card-outline">
        <div class="card-header">
            <h3 class="card-title">List <a href="{{ route('seller.create') }}"><span
                    ...
                    @else
                        @foreach ($data as $key => $val)
                            <tr>
                                ...
                                <td>{{ $val->name }}</td>
                                <td>{{ $val->tel }}</td>
                                <td>{{ isset($val->seller_type->name)?$val->seller_type->name:''}}</td>
                                <td>{{ isset($val->brand->name)?$val->brand->name:'' }}</td>
                                <td>

                                    <form action="{{ route('seller.destroy', $val->id) }}" method="POST">
                                        <a href="{{ route('seller.show', $val->id) }}" class="btn btn-primary "> <i
                                                class="fa-sharp fa-solid fa-magnifying-glass">&nbsp;</i>View</a>
                                        <a href="{{ route('seller.edit', $val->id) }}" class="btn btn-warning"> <i
